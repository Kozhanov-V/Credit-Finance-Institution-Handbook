<template>
	<div class="signup">
			...
			<form @submit.prevent="submitForm">
				...
				<div class="input-group">
  <label for="phone"><h2>НОМЕР ТЕЛЕФОНА</h2></label>
  ...
  <div v-if="v$.user.phone.$error" class="error">Введите телефон в формате "(999) 999-99-99"</div>
				...
				<div class="input-group">
					<label for="password"><h2>ПОДТВЕРДИТЕ ПАРОЛЬ</h2></label>
					<input type="password" placeholder="сonfirm password" name="password" id="confirm_password" v-model="confirmPassword">
					<div v-if="v$.confirmPassword.$error" class="error">Пароли должны совпадать</div>
				</div>
					<input type="submit" value="ЗАРЕГЕСТРИРОВАТЬСЯ" />
					<p>Есть аккаунта? <a href="/login">Войти</a></p>
					<p style="font-size: calc(var(--index)*0.3); width: calc(var(--index)*10); text-align: center; margin-top: calc(var(--index)*0.2);">Нажимая "ЗАРЕГЕСТРИРОВАТЬСЯ" вы принимаете пользовательское соглашение и политику конфиденциальности</p>
			</form>
			<div v-if="showConfirmation">
			<label for="confirmationCode">Введите код подтверждения из SMS</label>
			<input type="text" id="confirmationCode" v-model="confirmationCode">
      ...
      if (this.v$.$invalid) {
        return
			...
			if (response.data==="A confirmation code has been sent to your phone number") {
				...
				alert('Данный username или номер телефона занят')
